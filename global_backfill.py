...
def global_fetch(begin_time="2024-01-01"):
    client = EastMoneyReportClient()
    end_time = datetime.now().strftime('%Y-%m-%d')
    
    print(f"Global fetch from {begin_time} to {end_time}...")
    ...
    while page_no <= total_pages:
        try:
            ...
            if page_no == 1:
                ...
                if total_pages == 0:
                    total_pages = 1
                print(f"Total global pages to fetch: {total_pages}")
            ...
            if page_no % 50 == 0:
                print(f"Fetched {page_no}/{total_pages} pages...")
    ...
    print(f"\nFetched {sum(len(v) for v in industry_groups.values())} total reports across {len(industry_groups)} industries.")
    ...
    print(f"Total new records appended: {total_new_reports}")
...
if __name__ == "__main__":
    ...
    global_fetch(begin_time="2024-01-01")
